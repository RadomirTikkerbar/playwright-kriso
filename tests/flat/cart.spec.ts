...
test.describe('Add Books to Shopping Cart', () => {
  ...
  test('Test search by keyword', async () => {
    await page.getByRole('textbox', { name: 'Pealkiri, autor, ISBN, märksõna' }).first().click();
    await page.getByRole('textbox', { name: 'Pealkiri, autor, ISBN, märksõna' }).first().fill('harry potter');
